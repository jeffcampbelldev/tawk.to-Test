...
class UserListInvertedCell: UITableViewCell {
...
extension UserListInvertedCell: CellConfigurable {
    func configure(withUser user: RowViewModel){
        if let user = user as? UserListInvertedModel {
            initUI()
                ...
                NetworkManager.shared.loadData(
                    ...
                    MAIN_QUEUE.async { [weak self] in
                        if let data = data {
                            ...
                            self?.avatarImageView.image = image?.invertedImage()
                        } else {
